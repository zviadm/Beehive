...
//ARPipAddress =  0xc0a80002 //192.168.0.2 - Cambridge TFTP
...
decwords = 10
hexwords = 8
.blkw 128  //place the tables above the save/restore core code when debugging
...
//wait for a message
waitMsg:
    long_ld    count, 5000000    //Timeout. ~10 cycles/iteration.
    call       receiveMessage
    ld         Temp, result
...
/*
    long_ld   ma1, (packetBuffer - data) + 0x1c
    call      byteSwap //gets the block number into bits 15:0
    and       Temp, Temp, LowHalf
    sub       Temp, Temp, 1
    lsl       Temp, Temp, 9  //(block number - 1) * 512
    long_ld   t1, 0x3ffc
...
/*  
//wait for a reply
   long_ld   count, 10000  //timeout count
   call      receiveMessage
   ld        Temp, result  //length, or zero if timeout
   jz        timeout
   sub       void, result, 1  //length should be 1
   jnz        badLength
   sub       t1, t1, 1    //t1 has source core
   xor       void, t1, EtherCore  //must have come from Ethercore + 1
   jnz       wrongCore
*/
