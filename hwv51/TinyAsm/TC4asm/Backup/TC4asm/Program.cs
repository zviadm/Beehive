...
/*                            if (mx == 0) //format printing of IM
                            {
                                Console.WriteLine(" {0}: Rw = {1}, Ra = {2}, Rb = {3}, F = {4}, Sh = {5}, Sk = {6}, Op = {7}",
                                        i, //location
                                        (x >> 27) & 0x7f, //Rw
                                        x >> 20 & 0x7f, //Ra
                                        x >> 13 & 0x7f, //Rb
                            ...
                            if (pass == 1)
                            {
                                ...
                                if (i == 1)  //may appear only as the second token on a line
                                {
                                    if (tokens[i - 1].type == 1) //must be a string  //this is the key for the Symbol
                                    {
                                        ...
                                        Symbol sq = (Symbol)Symbols["boff"];

                                        Symbol s = new Symbol((ulong)cloc, (int) sq.value);
